...
app.get('/restaurant', function(req,res){
	...
	if (!req.session.authenticated) {
		...
		req.session.name = req.query._id;
		console.log('id: '+ req.session.name);
		MongoClient.connect(mongourl, function(err, db) {
			assert.equal(err,null);
			console.log('Connected to MongoDB\n');
			console.log("displayid: "+req.session.name);
			db.collection('restaurant').findOne({name: req.session.name},function(err,doc) {
				console.log('doc: '+JSON.stringify(doc));
				assert.equal(err,null);
				db.close();
				console.log('Disconnected from MongoDB\n');
				res.status(200);
				res.render('restaurant',{name:doc.name, borough:doc.borough,cuisine:doc.cuisine,grade:doc.grade, owner:doc.owner, building:doc.address.building, street:doc.address.street, lat: doc.address.lat, long:doc.address.long});
			});
		});
	}
});
...
function read_n_print(res,criteria,max,callback) {
		...
		findRestaurants(db,criteria,max,function(restaurants) {
			...
			if (restaurants.length == 0) {
				...
				restaurants[i].name+'>'+restaurants[i].name+
				'</a></li>');
				}
...
function updateRestaurant(req,db,newValues,callback) {
		...
		db.collection('restaurant').updateOne({name: req.session.name},{$set: newValues},function(err,result){
		assert.equal(err,null);
...
function deleteRestaurant(db,req,callback) {
	...
	db.collection('restaurant').deleteOne({ name:req.session.name, owner: req.session.username },function(err,result) {
		assert.equal(err,null);
